---
layout: TheoryContentLayout
kicker: Formules
title: Saisir un calcul
slide_info: false
---

Excel fait la différence entre une valeur écrite et une formule.

```excel
12*4        // affiché comme du texte ou une saisie simple
=12*4       // calculé par Excel
=C2*D2      // calcul à partir de deux cellules
```

À retenir :

- Une formule commence par `=`.
- Une formule peut utiliser des valeurs (nombres, textes, dates, etc) ou des cellules.
- Le résultat affiché dépend des cellules utilisées : si une valeur change, le calcul change.
